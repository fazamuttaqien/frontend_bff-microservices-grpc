...
function messageOf(error: unknown) { return error instanceof ApiError || error instanceof Error ? error.message : 'Unable to load orders.' }
async function fetchList(token: string, page: number, pageSize: number) {
  const key = `${page}:${pageSize}`; const cached = listCache.get(key); if (cached) return cached
  const existing = listRequests.get(key); if (existing) return existing
  ...
  listRequests.set(key, request); return request
}
async function fetchDetail(token: string, id: string) {
  const cached = detailCache.get(id); if (cached) return cached
  const existing = detailRequests.get(id); if (existing) return existing
...
export function invalidateOrderCache() { listCache.clear(); detailCache.clear() }
export function clearOrderCache(id?: string) { if (id) detailCache.delete(id); else invalidateOrderCache() }
...
export function useOrders(page = 1, pageSize = 20) {
  ...
  useEffect(() => { void load() }, [load]); return { orders, total, loading, error, reload: load }
}
