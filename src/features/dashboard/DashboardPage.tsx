...
export function DashboardPage() {
  ...
  const { data: featured, loading: detailLoading, error: detailError, reload: reloadDetail } = useOrder(latestOrderId)

  if (loading) return <section className="page" aria-busy="true"><h1>Dashboard</h1><OrderLoading /></section>
  ...
  return <section className="page">
    ...
    {error ? <OrderError message={error} onAction={() => void reload()} /> : <>
      ...
      {latestOrderId && detailLoading && <section aria-busy="true"><OrderLoading /></section>}
      {featured && !detailLoading && <section aria-labelledby="dashboard-aggregation"><h2 id="dashboard-aggregation">Latest order details</h2><p>Product information is supplied by the BFF aggregation endpoint.</p><OrderSummary order={featured.order} products={featured.products} /></section>}
      {detailError && !detailLoading && <section role="status"><p>Recent orders are available, but aggregated details could not be loaded: {detailError}</p><button onClick={() => void reloadDetail()}>Retry details</button></section>}
    </>}
  </section>
}
